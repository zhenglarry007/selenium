...
public class OpenNanaDataExtractionTest extends BaseWeb {
    ...
    public void extractPromptData() {
        ...
        String jsonFilePath = OUTPUT_DIRECTORY + "/" + DataSaver.generateOutputFilePath(OUTPUT_FILE_NAME);
        DataSaver.saveToJson(extractedData, jsonFilePath);

        String csvFilePath = OUTPUT_DIRECTORY + "/" + DataSaver.generateOutputFilePath(OUTPUT_FILE_NAME).replace(".json", ".csv");
    ...
    @Description("Extract data from first card only for debugging purposes")
